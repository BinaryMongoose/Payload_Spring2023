...
void setup() {
  Serial.begin(115200);
  Serial.println(F(" ===== Welcome to the Real Deal =====\n"));

  if(!SD.begin(4)){
    halt(F("Failed to open SD Card!"));
  } else {
...
void loop() {
  ...
  if (millis() - timer > 5000) {
    ...
    if (GPS.fix) {
      ...
      test.print(GPS.latitudeDegrees, 10); test.print(GPS.lat);   // Laatitude
      test.print(",");
      test.print(GPS.longitudeDegrees, 10); test.print(GPS.lon);  // Longitude 
      test.print(",");
      test.print(GPS.speed * 1.151, 5);  // Speed
      test.print(",");
